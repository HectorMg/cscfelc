...
<!DOCTYPE html>
<title>FELC - The Bank: Food 2</title>
<html lang="en">
  <head>
    <meta charset="utf-8">
    ...
    <div class="container-fluid">
      <div class="row">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
          <h1 class="page-header">The Bank: <span class="text-muted">Food 2 Corporate Account</span></h1>
                ...
                <table class="table table-striped">
                  <thead>
                    <tr>
                      ...
                      $countRow = mysql_query("SELECT * FROM u404044402_bank . f2Trans");
                      $count = mysql_num_rows($countRow);

                      for($i = $count; $i >= 1; $i--){

                        //GET Trans. ID:
                        $getTransIDRes = mysql_query("SELECT Transaction_ID FROM u404044402_bank . f2Trans WHERE ID = $i");
                        $getTransIDRow = mysql_fetch_row($getTransIDRes);
                        $TransID = $getTransIDRow[0];
                        //GET DESCRIPTION:
                        $getDescRes = mysql_query("SELECT Description FROM u404044402_bank . f2Trans WHERE ID = $i");
                        $getDescRow = mysql_fetch_row($getDescRes);
                        $Desc = $getDescRow[0];
                        //GET WITHDRAWN FROM
                        $getWithdrawnRes = mysql_query("SELECT Company_Acquiring FROM u404044402_bank . f2Trans WHERE ID = $i");
                        $getWithdrawnRow = mysql_fetch_row($getWithdrawnRes);
                        $withdrawn = $getWithdrawnRow[0];
                        //GET DEPOSIT INTO
                        $getDepositRes = mysql_query("SELECT Company_Selling FROM u404044402_bank . f2Trans WHERE ID = $i");
                        $getDepositRow = mysql_fetch_row($getDepositRes);
                        $deposit = $getDepositRow[0];
                        //GET AMOUNT
                        $getAmountRes = mysql_query("SELECT Amount FROM u404044402_bank . f2Trans WHERE ID = $i");
                        $getAmountRow = mysql_fetch_row($getAmountRes);
                        $amount = $getAmountRow[0];
                        //GET PREV ACC BAL
                        $getPrevBalRes = mysql_query("SELECT Previous_Balance FROM u404044402_bank . f2Trans WHERE ID = $i");
                        $getPrevBalRow = mysql_fetch_row($getPrevBalRes);
                        $PrevBal = $getPrevBalRow[0];
                        //GET NEW ACC BAL
                        $getBalRes = mysql_query("SELECT New_Balance FROM u404044402_bank . f2Trans WHERE ID = $i");
                    ...
                    <tbody>
                      <tr id="bTrans">
                        ...
                        for($i = 1; $i <= 8; $i++){
                          ...
                          //GET Si:
                          $getsharesRes = mysql_query("SELECT `$i` FROM u404044402_bank . companyAccounts WHERE ID = 2");
      ...
      <!-- Bank Transactions -->
            ...
            <div class="panel panel-success">
              ...
              <tbody id="OwnedProp">
                       <tr>
                          <td><form action="actions/depositF2.php" method="post"><input style="height: 30px; width: 200px;" type="number" step= "any" placeholder="Insert Amount" name="Amount" class="form-control"></td>
                       ...
                       <?php } elseif($_GET['deposit'] == "e1"){ ?>
                          ...
                          $resultID = mysql_query("SELECT MAX(ID) FROM u404044402_bank . f2Trans");
                          $rowID= mysql_fetch_row($resultID);
                          $resultBal = mysql_query("SELECT New_Balance FROM u404044402_bank . f2Trans WHERE ID = '$rowID[0]'");
              ...
              <tbody>
                       <tr>
                          <td><form action="actions/withdrawF2.php" method="post"><input style="height: 30px; width: 200px;" type="number" step= "any" placeholder="Insert Amount" name="Amount" class="form-control"></td>
