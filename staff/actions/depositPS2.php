...
$prevID = $rowID[0];
$resultPrevBal = mysql_query("SELECT Balance FROM companyAccounts WHERE ID = 6");
...
//Query Deposit ps2Trans
$depositQuery = "INSERT INTO ps2Trans (ID, Transaction_ID, Description, Company_Acquiring, Company_Selling, Amount, Previous_Balance, New_Balance, Type) VALUES (NULL, '$newID', 'Deposit', '$from', '$to', '$amount', '$prevBal', '$newBal', 'Deposit')";
...
$balanceQuery = mysql_query("UPDATE companyAccounts SET PreviousBalance = $prevBal, Balance = $newBal WHERE ID = 6");

if($deposit && $cbTransQuery && $balanceQuery){
            header("location:../bankPS2.php?deposit=e1#bTrans");
        }
        else
        {
            die (mysql_error());
            header("location:../bankPS2.php?deposit=error#bTrans");
        }
