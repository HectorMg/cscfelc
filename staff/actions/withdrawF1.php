...
$prevID = $rowID[0];
$resultPrevBal = mysql_query("SELECT Balance FROM companyAccounts WHERE ID = 1");
...
//Query Withdrawal f1Trans
$withdrawQuery = "INSERT INTO f1Trans (ID, Transaction_ID, Description, Company_Acquiring, Company_Selling, Amount, Previous_Balance, New_Balance, Type) VALUES (NULL, '$newID', 'Withdrawal', '$from', '$to', '$amount', '$prevBal', '$newBal', 'Withdrawal')";
...
$balanceQuery = mysql_query("UPDATE companyAccounts SET PreviousBalance = $prevBal, Balance = $newBal WHERE ID = 1");


if($withdraw){
            header("location:../bankF1.php?withdraw=e1#bTrans");
        }
        else
        {
            die (mysql_error());
            header("location:../bankF1.php?withdraw=error#bTrans");
        }
