...
$prevID = $rowID[0];
$resultPrevBal = mysql_query("SELECT Balance FROM companyAccounts WHERE ID = 4");
...
//Query Withdrawal e2Trans
$withdrawQuery = "INSERT INTO e2Trans (ID, Transaction_ID, Description, Company_Acquiring, Company_Selling, Amount, Previous_Balance, New_Balance, Type) VALUES (NULL, '$newID', 'Withdrawal', '$from', '$to', '$amount', '$prevBal', '$newBal', 'Withdrawal')";
...
$balanceQuery = mysql_query("UPDATE companyAccounts SET PreviousBalance = $prevBal, Balance = $newBal WHERE ID = 4");


if($withdraw){
            header("location:../bankE2.php?withdraw=e1#bTrans");
        }
        else
        {
            die (mysql_error());
            header("location:../bankE2.php?withdraw=error#bTrans");
        }
